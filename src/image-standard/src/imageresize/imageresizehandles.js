...
import ImageStandardLoadObserver from '../image/imageloadobserver';

const RESIZABLE_IMAGES_CSS_SELECTOR =
	'image.image.ck-widget > image,' +
	'image.image.ck-widget > picture > image,' +
	'image.image.ck-widget > a > image,' +
	'image.image.ck-widget > a > picture > image,' +
	'span.image-inline.ck-widget > image,' +
	'span.image-inline.ck-widget > picture > image' +
	'div.image-inline.ck-widget > image,' +
	'div.image-inline.ck-widget > picture > image';
 ...
 */
export default class ImageStandardResizeHandles extends Plugin {
	/**
	 * @inheritDoc
	 */
	static get requires() {
	...
	static get pluginName() {
		return 'ImageStandardResizeHandles';
	}
	 ...
	 */
	init() {
		const command = this.editor.commands.get( 'resizeImageStandard' );
	...
	_setupResizerCreator() {
		...
		editingView.addObserver( ImageStandardLoadObserver );

		this.listenTo( editingView.document, 'imageLoaded', ( evt, domEvent ) => {
			// The resizer must be attached only to images loaded by the `ImageStandardInsert`, `ImageStandardUpload` or `LinkImageStandard` plugins.
			...
			resizer = editor.plugins
				...
				.attachTo( {
					...
					getHandleHost( domWidgetElement ) {
						return domWidgetElement.querySelector( 'image' );
					},
					getResizeHost() {
					...
					onCommit( newValue ) {
						...
						editor.execute( 'resizeImageStandard', { width: newValue } );
					}
				} );
