 ...
 */
export default class AutoImageStandard extends Plugin {
	/**
	 * @inheritDoc
	 */
	static get requires() {
		return [ Clipboard, ImageStandardUtils, Undo, Delete ];
	}
	...
	constructor( editor ) {
		...
		/**
		 * The position where the `<imageStandardBlock>` element will be inserted after the timeout,
	...
	init() {
		...
		this.listenTo( editor.plugins.get( 'ClipboardPipeline' ), 'inputTransformation', () => {
			...
			modelDocument.once( 'change:data', () => {
				this._embedImageStandardBetweenPositions( leftLivePosition, rightLivePosition );
	 ...
	 */
	_embedImageStandardBetweenPositions( leftPosition, rightPosition ) {
		const editor = this.editor;
		...
		this._timeoutId = global.window.setTimeout( () => {
			...
			const imageCommand = editor.commands.get( 'insertImageStandard' );
			...
			editor.model.change( writer => {
				...
				// Otherwise leave it as undefined to use the logic of insertImageStandard().
				if ( this._positionToInsert.root.rootName !== '$graveyard' ) {
					insertionPosition = this._positionToInsert.toPosition();
				}

				imageUtils.insertImageStandard( { ...selectionAttributes, src }, insertionPosition );
