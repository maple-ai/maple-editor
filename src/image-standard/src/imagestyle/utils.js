...
/**
 * Default image style options provided by the plugin that can be referred in the {@link module:image/image~ImageStandardConfig#styles}
 * configuration.
 *
 ...
 *
 * @readonly
 * @type {Object.<String,module:image/imagestyle~ImageStandardStyleOptionDefinition>}
 */
...
/**
 * Default image style icons provided by the plugin that can be referred in the {@link module:image/image~ImageStandardConfig#styles}
 * configuration.
 *
 * See {@link module:image/imagestyle~ImageStandardStyleOptionDefinition#icon} to learn more.
 *
...
/**
 * Default drop-downs provided by the plugin that can be referred in the {@link module:image/image~ImageStandardConfig#toolbar}
 * configuration. The drop-downs are containers for the {@link module:image/imagestyle~ImageStandardStyleConfig#options image style options}.
 *
 * If both of the `ImageStandardEditing` plugins are loaded, there are 2 predefined drop-downs available:
 *
 ...
 *
 * @readonly
 * @type {Array.<module:image/imagestyle/imagestyleui~ImageStandardStyleDropdownDefinition>}
 */
 ...
 * Determines whether the {@link module:image/image/imageblockediting~ImageStandardBlockEditing `ImageStandardBlockEditing`} plugin has been loaded.
 * @param {Boolean} config.isBlockPluginLoaded
 * Determines whether the {@link module:image/image/imageinlineediting~ImageStandardInlineEditing `ImageStandardInlineEditing`} plugin has been loaded.
 * @param {module:image/imagestyle~ImageStandardStyleConfig} config.conimagedStyles
 * The image styles configuration provided in the image styles {@link module:image/image~ImageStandardConfig#styles configuration}
 * as a default or custom value.
 * @returns {module:image/imagestyle~ImageStandardStyleConfig}
...
function normalizeStyles( config ) {
	const conimagedStyles = config.conimagedStyles.options || [];

	const styles = conimagedStyles
 ...
 * @returns {Array.<module:image/imagestyle/imagestyleui~ImageStandardStyleDropdownDefinition>}
 */
function getDefaultDropdownDefinitions( pluginCollection ) {
	if ( pluginCollection.has( 'ImageStandardBlockEditing' ) && pluginCollection.has( 'ImageStandardInlineEditing' ) ) {
...
// Normalizes an image style option or group provided in the {@link module:image/image~ImageStandardConfig#styles}
// and returns it in a {@link module:image/imagestyle~ImageStandardStyleOptionDefinition}/
//
// @param {Object|String} definition
//
// @returns {module:image/imagestyle~ImageStandardStyleOptionDefinition}}
...
//
// @param {module:image/imagestyle~ImageStandardStyleOptionDefinition} image style option
...
function isValidOption( option, { isBlockPluginLoaded, isInlinePluginLoaded } ) {
	...
	if ( !modelElements || !modelElements.length || !name ) {
		...
		const supportedElements = [ isBlockPluginLoaded ? 'imageStandardBlock' : null, isInlinePluginLoaded ? 'imageStandardInline' : null ];
		...
		if ( !modelElements.some( elementName => supportedElements.includes( elementName ) ) ) {
			/**
			 * In order to work correctly, each image style {@link module:image/imagestyle~ImageStandardStyleOptionDefinition option}
			 ...
			 * {@link module:image/imagestyle~ImageStandardStyleOptionDefinition#modelElements} property of the style option
			 * definition.
			 *
			...
			logWarning( 'image-style-missing-dependency', {
				style: option,
				missingPlugins: modelElements.map( name => name === 'imageStandardBlock' ? 'ImageStandardBlockEditing' : 'ImageStandardInlineEditing' )
			} );
...
//
// @param {module:image/imagestyle~ImageStandardStyleOptionDefinition} source
// @param {Object} style
//
// @returns {module:image/imagestyle~ImageStandardStyleOptionDefinition}
...
function warnInvalidStyle( info ) {
	 ...
	 *
	 * * {@link module:image/imagestyle~ImageStandardStyleOptionDefinition image style option definition},
	 * * {@link module:image/imagestyle/imagestyleui~ImageStandardStyleDropdownDefinition image style dropdown definition}
	 *
