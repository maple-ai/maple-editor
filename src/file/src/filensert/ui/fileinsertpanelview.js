...
/**
 * @module file/fileinsert/ui/fileinsertpanelview
 */
...
import FileInsertFormRowView from './fileinsertformrowview';

import '../../../theme/fileinsert.css';

/**
 * The insert an file via URL view controller class.
 *
 * See {@link module:file/fileinsert/ui/fileinsertpanelview~FileInsertPanelView}.
 *
 * @extends module:ui/view~View
 */
export default class FileInsertPanelView extends View {
	/**
	 * Creates a view for the dropdown panel of {@link module:file/fileinsert/fileinsertui~FileInsertUI}.
	 *
	...
	constructor( locale, integrations ) {
		 ...
		 *
		 * @member {String} #fileURLInputValue
		 * @observable
		 */
		this.set( 'fileURLInputValue', '' );
		...
		/**
		 * A collection of the defined integrations for inserting the files.
		 *
		 * @private
		...
		if ( integrations ) {
			for ( const [ integration, integrationView ] of Object.entries( integrations ) ) {
				if ( integration === 'insertFileViaUrl' ) {
					integrationView.fieldView.bind( 'value' ).to( this, 'fileURLInputValue', value => value || '' );

					integrationView.fieldView.on( 'input', () => {
						this.fileURLInputValue = integrationView.fieldView.element.value.trim();
					} );
				}
		...
		this.setTemplate( {
			...
			children: [
				...this._integrations,
				new FileInsertFormRowView( locale, {
					children: [
						this.insertButtonView,
						this.cancelButtonView
					],
					class: 'ck-file-insert-form__action-row'
				} )
			]
		} );
	}
	...
	_createActionButtons( locale ) {
		...
		insertButtonView.set( {
			...
			type: 'submit',
			withText: true,
			isEnabled: this.fileURLInputValue
		} );
		...
		insertButtonView.bind( 'isEnabled' ).to( this, 'fileURLInputValue', value => !!value );
