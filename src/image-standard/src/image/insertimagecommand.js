 ...
 *
 * The command is registered by the {@link module:image/image/imageediting~ImageStandardEditing} plugin as `insertImageStandard`
 ...
 *
 *		editor.execute( 'insertImageStandard', { source: 'http://url.to.the/image' } );
 *
 * It is also possible to insert multiple images at once:
 *
 *		editor.execute( 'insertImageStandard', {
 *			source:  [
 *				'path/to/image.jpg',
 ...
 */
export default class InsertImageStandardCommand extends Command {
	/**
	 * @inheritDoc
	 */
	constructor( editor ) {
		...
		if ( !editor.plugins.has( 'ImageStandardInlineEditing' ) ) {
			if ( configImageStandardInsertType === 'inline' ) {
				/**
				 * The {@link module:image/imageinline~ImageStandardInline} plugin must be enabled to allow inserting inline images. See
				 * {@link module:image/imageinsert~ImageStandardInsertConfig#type} to learn more.
				 *
		...
		if ( !editor.plugins.has( 'ImageStandardBlockEditing' ) ) {
			if ( configImageStandardInsertType === 'block' ) {
				/**
				 * The {@link module:image/imageblock~ImageStandardBlock} plugin must be enabled to allow inserting block images. See
				 * {@link module:image/imageinsert~ImageStandardInsertConfig#type} to learn more.
				 *
	 ...
	 */
	refresh() {
		this.isEnabled = this.editor.plugins.get( 'ImageStandardUtils' ).isImageStandardAllowed();
	}
	...
	execute( options ) {
		...
		sourceDefinitions.forEach( ( sourceDefinition, index ) => {
			...
			if ( index && selectedElement && imageUtils.isImageStandard( selectedElement ) ) {
				...
				imageUtils.insertImageStandard( { ...sourceDefinition, ...selectionAttributes }, position );
			} else {
				imageUtils.insertImageStandard( { ...sourceDefinition, ...selectionAttributes } );
			}
		} );
	}
}
