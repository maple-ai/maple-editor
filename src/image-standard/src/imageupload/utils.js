 ...
 * @returns {RegExp}
 */
export function createImageStandardTypeRegExp( types ) {
 ...
 *
 * @param {module:engine/view/element~Element} image ImageStandard whose source to fetch.
 ...
 */
export function fetchLocalImageStandard( image ) {
	return new Promise( ( resolve, reject ) => {
		...
		fetch( imageSrc )
			...
			.then( blob => {
				const mimeType = getImageStandardMimeType( blob, imageSrc );
			...
			.catch( err => {
				...
				return err && err.name === 'TypeError' ?
					convertLocalImageStandardOnCanvas( imageSrc ).then( resolve ).catch( reject ) :
					reject( err );
			} );
	} );
}
 ...
 * @returns {Boolean}
 */
export function isLocalImageStandard( imageUtils, node ) {
	if ( !imageUtils.isInlineImageStandardView( node ) || !node.getAttribute( 'src' ) ) {
		return false;
	}
...
//
// @param {String} src ImageStandard `src` attribute value.
// @param {Blob} blob ImageStandard blob representation.
// @returns {String}
function getImageStandardMimeType( blob, src ) {
	if ( blob.type ) {
		return blob.type;
...
// with a `File` object.
//
// @param {String} imageSrc ImageStandard `src` attribute value.
// @returns {Promise.<File>} A promise which resolves when an image source is converted to a `File` instance.
// It resolves with a `File` object. If there were any errors during file processing, the promise will be rejected.
function convertLocalImageStandardOnCanvas( imageSrc ) {
...
function getBlobFromCanvas( imageSrc ) {
	return new Promise( ( resolve, reject ) => {
		...
		image.addEventListener( 'load', () => {
			...
			ctx.drawImageStandard( image, 0, 0 );
